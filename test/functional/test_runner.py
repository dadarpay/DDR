...
def main():
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and ddrd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, keep_cache=False):
    # Warn if ddrd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "ddrd"]) is not None:
            print("%sWARNING!%s There is already a ddrd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/ddrd' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/ddr-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `ddr-cli help` (`rpc_interface.txt`).
