...
{
    if (nHeight > 1300000)	return 20000 * COIN;
    if (nHeight > 1000000)	return 15000 * COIN;
    if (nHeight > 600000)	return 10000 * COIN;
    if (nHeight > 500000)	return 7500 * COIN;
...
{
    if (nHeight > 4000000)	return 2   * COIN;
    if (nHeight > 3000000)	return 5   * COIN;
    if (nHeight > 2000000)	return 15   * COIN;
    if (nHeight > 1600000)	return 25   * COIN;
    if (nHeight > 1300000)	return 50   * COIN;
    if (nHeight > 1000000)	return 40   * COIN;
    if (nHeight > 750000)	return 30   * COIN;
    if (nHeight > 600000)	return 25   * COIN;
    if (nHeight > 500000)	return 20   * COIN;
    if (nHeight > 300000)	return 18   * COIN;
    if (nHeight > 200000)	return 15   * COIN;
    if (nHeight > 100000)	return 10   * COIN;
    if (nHeight > 50000)	return 5   * COIN;
    if (nHeight >= 1)		return 2 * COIN;
    return 2 * COIN;
}
...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    return GetBlockValue(nHeight) * percentage / 100;
}

CAmount CMasternode::GetDevPayment(int nHeight)
{
    return GetBlockValue(nHeight) * 5 / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 DDR tx -> 1 confirmation
