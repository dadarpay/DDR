...
const char * const PIVX_CONF_FILENAME = "ddr.conf";
const char * const PIVX_PID_FILENAME = "ddr.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ddr
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ddr
// Mac: ~/Library/Application Support/ddr
// Unix: ~/.ddr
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dadar";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DadarParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "DadarParams";
#else
    // Unix
    return pathRet / ".ddr-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ddr.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Dadar config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ddr.conf
